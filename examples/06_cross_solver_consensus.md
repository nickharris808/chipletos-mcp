...
- `solver_status` block — which solvers have 100/100 coverage on the
  100-geometry sweep
- `honest_caveat` — current a recent release for partial solver coverage
- `n_geometries` and `content_sha256` for reproducibility

**Heads-up:** a recent release Palace freq extension measured a 6.73 % median
BEM-vs-Palace freq-spread envelope across 25 geoms × 4 freqs; a recent release
refined this to ~17.3 % median across 33 geoms × 6 freqs. a recent release v2-clean
